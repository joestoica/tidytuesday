...
top20$drink <- factor(top20$drink, levels = top20$drink)

top20 %>% 
  ggplot(aes(reorder(country, qty),
             qty, 
             fill = drink, 
             order = as.numeric(drink))) +
  ...
  guides(fill = guide_legend(keywidth = 3, keyheight = .75, title=NULL, label.position = "top", label.hjust = 0.5,
